...
public:
   ...
   * time you call it.
   */
  framebuffer(void) : framebufferID(0), framebufferIDcopied(false) {}
...
public:
   ...
   * \return True on success, false otherwise.
   */
  bool load(const GLuint &pWidth, const GLuint &pHeight, const void *data = 0) {
    if (bind()) {
      if ((pWidth != width) || (pHeight != height) || (data != 0)) {
        width = roundToPowerOf2(pWidth);
        height = roundToPowerOf2(pHeight);

        glTexImage2D(target, 0, format, width, height, 0, baseFormat, type,
                     data);
      }
...
public:
   ...
   * time you call it.
   */
  renderbuffer(void) : renderbufferID(0) {}
 ...
 *      the possible values of the target, format and type parameters.
 */
template <typename Q, GLenum format = GL_RGB, GLenum baseFormat = format,
...
public:
  ...
  bool use(const GLuint &width, const GLuint &height) {
    if (framebuffer<Q>::use() &&
        texture<target, format, baseFormat, type>::load(width, height)) {
      glTexParameteri(target, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
      glTexParameteri(target, GL_TEXTURE_MIN_FILTER, GL_NEAREST);

      glFramebufferTexture2D(
          GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, target,
          texture<target, format, baseFormat, type>::textureID, 0);
...
public:
  ...
  bool use(const GLuint &width, const GLuint &height,
    ...
    if (programme<Q, V, vertexShader, fragmentShader>::use() &&
        framebufferTexture<Q, format, baseFormat, type, target>::use(width,
                                                                     height)) {
...
public:
   ...
   * use() method.
   */
  vertexArray(void) : vertexArrayID(0) {}
