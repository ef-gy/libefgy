...
namespace efgy {
namespace optimise {
template <typename T, int genomeLength, typename Fitness, typename Mutate,
          typename hasTerminated, template <typename S> class Select,
          typename Initialise, int populationSize = 1000, typename Q = double>
...
/*
        ...
        This class contains the entire population of the GA; individual
    genomes are
        represented as individual<T, genomeLength>.

        \tparam Fitness is assumed to have a _static_ overloaded () operator
    that accepts
        a individual and returns a value of type Q.

        \tparam Q is assumed to be a type that represents rational numbers;
    the default is double,
        but one might want to use long doubles, fractions, or other rational
    data types instead.
        Since values of type Q are used to represent probabilities, please
    bear in mind
        that Q should be able to represent values between 0 and 1, so
    integers are not a good choice.

        \tparam Mutate is assumed to be a functor with a static overloaded
    () operator that accepts an argument of type T and return
        a value of type T. The functor may either mutate its argument
    explicitly (for example, apply
        the NOT operator to a boolean) or simply return a random value of
    type T, but users of this class
        should assume that the functor does not return its argument
    unchanged.

        \tparam hasTerminated is assumed to be a functor with a static
    overloaded () operator
         which takes no arguments and returns a boolean value that
        indicates whether the genetic algorithm should terminate.

        \tparam Select is assumed to be a functor with a static overloaded
    () operator that takes the target population size,
        a vector of individual and the Fitness functor as its arguments,
        and returns a vector of individual as the new population,
    implementing a selection
        method of the user's choice. Its template argument is an instance of
    type geneticAlgorithm,
        usually the this pointer.

        \tparam Initialise is assumed to be a functor that takes an array of
    T
...
class geneticAlgorithm {
 ...
 protected:
  ...
  std::pair<individual, individual> onePointCrossover(individual &i1,
                                                      individual &i2) {
    std::uniform_int_distribution<> dis(0, genomeLength);
    int position = dis(rng);
 ...
 public:
  ...
  individual start() {
    ...
    for (typename std::vector<individual>::iterator it = population.begin();
...
class SelectTournament {
 public:
  ...
  std::vector<typename S::individual> operator()(
    ...
    for (typename std::vector<typename S::individual>::iterator it =
             population.begin();
         it != population.end(); it++) {
      typename S::FitnessFunctor fitness;
      current.insert(std::pair<typename S::base, typename S::individual>(
          fitness(*it), *it));
    }
    ...
    while (newPopulation.size() < targetSize) {
      ...
      for (int i = 0; i < tournamentSize; i++) {
        ...
        int r = dis(rng);
        {
          typename std::map<typename S::base, typename S::individual>::iterator
              it = current.begin();
          for (int k = 0; (it != current.end()) && (k < r); k++, it++) {
        ...
        for (int i = 0; i < tournamentSize; i++) {
          typename std::map<typename S::base, typename S::individual>::iterator
              it;
          it = tournament.upper_bound(
              std::numeric_limits<typename S::base>::max());
